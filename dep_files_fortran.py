""" Find arg[1].f90 file in the arg[2] directory (or its subdirs), generate list of "use"d .f90 files, and write them as a JSON string to he.json.

INPUTS:
arg[1] - file name (without .f90) to start from. We'll read it and see all call's and use's it has. Then we will open the first call and do the same recursively until all call's and use's were analyzed. For example main.f90 would be arg[1]=main
arg[2] - highest level directory you want to allow to be searched (assuming main calls and uses other files in other subfolders).

OUTPUTS:
(several files will be written:)
dot.gv - graph file understood by dot
d.json - file understood by d3js
g.json - also some variation for d3js
gl.json 
forced.json - also for d3js, but for forced layout
forced.css - colors and radii for d3js forced layout

DEPENDENCIES:
wc - unix command for word/linecount (also works in cygwin)
grep - for searching inside files
dot - not called inside this file, but you need to call afterwards to create an image

EXAMPLES: 
python dep_files_fortran.py main.f90 /Users/alex/HEAD/DART13/models/gitm/GITM2/
dot -Tsvg dot.gv -o gitm.svg
"""
